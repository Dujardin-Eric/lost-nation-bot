...
bot.on('ready', function() {
    ...
    //        regarde = WATCHING
    //
    bot.user.setActivity("Démineur", {type: "PLAYING"}).
    catch(console.error)
});
...
bot.on('guildMemberAdd', member => {
    ...
    member.guild.channels.cache.find(channel => channel.id === "528285622369255447").send(member.displayName + " est arrivé !");

});

bot.on('guildMemberRemove', member => {
    member.guild.channels.cache.find(channel => channel.id === "528285622369255447").send(member.displayName + " nous à quitté :sob:");
});
